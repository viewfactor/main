...
* SPECIAL, INCIDENTAL, INifle, OR CONSEQUENTIAL DAMAGES WHATSOEVER (INCLUDING, WITHOUT
...
void GLUTDisplay::run(const std::string& title, SampleScene* scene, contDraw_E continuous_mode)
{
	...
	glutIdleFunc(idle); // added for continous mode
...
void GLUTDisplay::mouseButton(int button, int state, int x, int y)
{
	...
	if (state != GLUT_UP)
		...
		m_scene->getContext()["click_coordinate"]->setFloat(make_float3(x, y, 1.0f));
	m_scene->getContext()["click_flag"]->setInt(1);
...
void GLUTDisplay::idle()	//Faizan did this to reshow main page after every simulations
{
	...
	glutPostRedisplay();
	*/
	//m_scene->getContext()["click_flag"]->setInt(0);

	if (count<1000) //7056//3530 //1120
	{
		m_scene->getContext()["count"]->setInt(count);
